...
class Qlean:
    ...
    def learn(self, state, action, reward, state1):
        q_next = max([self.getQ(state1, a) for a in self.actions])
        self.learnQ(state, action, reward, reward + self.gamma * q_next)
...
if __name__ == '__main__':
    ...
    env = RecordVideo(env, video_folder="./video_q",  episode_trigger=lambda t: t % 5000 == 0)
    n_bins = 8
    n_bins_angle = 10
    ...
    qlearn = Qlean(range(env.action_space.n), alpha=0.5, epsilon=0.1, gamma=0.9)
